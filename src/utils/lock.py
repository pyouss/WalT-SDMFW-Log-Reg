...
def is_locked():
    return os.path.exists(f"{ROOT_DIR}/.lock")

def lock_modifications():
    if not os.path.exists(f"{ROOT_DIR}/.lock"):
        with open(f"{ROOT_DIR}/.lock", 'w') as f:
            pass
...
def unlock_modifications():
    if os.path.exists(f"{ROOT_DIR}/.lock"):
        os.remove(f"{ROOT_DIR}/.lock")
